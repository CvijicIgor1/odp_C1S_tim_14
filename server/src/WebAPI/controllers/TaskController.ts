...
export class TaskController {
    ...
    private async getByProjectId(req: Request, res: Response): Promise<void> {
        ...
        const result = await this.taskService.getByProjectId(projectId, req.user!.user_id, req.user?.role === UserRole.ADMIN);
    ...
    private async update(req: Request, res: Response): Promise<void> {
        ...
        await this.auditService.log(req.user!.user_id, AuditAction.TASK_UPDATED, "tasks", id, undefined, req.ip);
    ...
    private async updateStatus(req: Request, res: Response): Promise<void> {
        ...
        await this.auditService.log(req.user!.user_id, AuditAction.TASK_STATUS_UPDATED, "tasks", id, `status: ${status}`, req.ip);
    ...
    private async delete(req: Request, res: Response): Promise<void> {
        ...
        await this.auditService.log(req.user!.user_id, AuditAction.TASK_DELETED, "tasks", id, undefined, req.ip);
    ...
    private async addAssignee(req: Request, res: Response): Promise<void> {
        ...
        const ok = await this.taskService.addAssignee(taskId, dto, req.user!.user_id);
        if (!ok) { res.status(400).json({ success: false, message: "Korisnik nije član ovog tima ili je već dodat" }); return; }

        await this.auditService.log(req.user!.user_id, AuditAction.TASK_ASSIGNEE_ADDED, "task_assignees", taskId, `assignedUserId: ${userId}`, req.ip);
    ...
    private async removeAssignee(req: Request, res: Response): Promise<void> {
        ...
        const ok = await this.taskService.removeAssignee(taskId, userId, req.user!.user_id);
        if (!ok) { res.status(404).json({ success: false, message: "Assignee not found" }); return; }

        await this.auditService.log(req.user!.user_id, AuditAction.TASK_ASSIGNEE_REMOVED, "task_assignees", taskId, `removedUserId: ${userId}`, req.ip);
    ...
    private async addComment(req: Request, res: Response): Promise<void> {
        ...
        const comment = await this.taskService.addComment(taskId, dto, req.user!.user_id);
        if (comment?.id === 0) { res.status(400).json({ success: false, message: "Failed to add comment or insufficient permissions" }); return; }

        await this.auditService.log(req.user!.user_id, AuditAction.COMMENT_ADDED, "comments", comment?.id, `taskId: ${taskId}`, req.ip);
    ...
    private async deleteComment(req: Request, res: Response): Promise<void> {
        ...
        await this.auditService.log(req.user!.user_id, AuditAction.COMMENT_DELETED, "comments", commentId, undefined, req.ip);
